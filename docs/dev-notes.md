...
8.  **Bare-Metal Deployment**: 
    ...
    - `make install`: Cross-platform command to deploy the application, install production dependencies (using modern `--omit=dev` flags), and configure NGINX and services. It also proactively clears deprecated global npm configs like `python` to ensure clean builds.
...
12. **Service Robustness**:
    - FreeBSD `rc` script now utilizes the `daemon` utility to background the process, manage PID files, and redirect STDOUT/STDERR to `/var/log/cloudbsd_website.log`. It uses `rc.subr`'s native user and environment handling (`_user`, `_chdir`, `_env`) instead of `daemon -u` to avoid issues with non-login shells (e.g., the `www` user). It includes a `prestart` routine to handle file permissions for the `www` user and sets the `HOME` environment variable to ensure `npm` operations remain localized and permission-safe. This ensures non-blocking startup and automatic recovery (`-r` flag) with full logging.
